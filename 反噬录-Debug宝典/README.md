...
## 本卷增强补全（2026）— 反噬定位 SOP：从现象到根因（建议做成团队 Runbook）

> 本节为《焚诀》深度研究版的**回填内容**，目标是把“Debug 经验”固化为可复用 SOP。  
> 完整方法论总览见：[焚诀-深度研究版-全卷增强补全（2026）](../焚诀-深度研究版-全卷增强补全.md)

### 1) 通用排查顺序（强烈推荐固定成习惯）

1. **复现**：最小复现脚本（去掉无关组件）  
2. **定位层级**：数据 → 模型 → 优化器/超参 → 分布式/硬件 → 服务  
3. **观测证据**：日志 + 指标（loss/grad norm/显存/延迟）+ 最近一次变更  
4. **对照实验**：回退到上一个稳定版本（数据/代码/权重/依赖）  
5. **修复与回归**：修复后必须加入回归用例，防止“治标不治本”

### 2) 三类高频反噬的“最小排查表”

- **Loss NaN/Inf**：混合精度溢出→grad 爆炸→log/exp 数值域→除零  
- **OOM**：seq_len/KV cache→激活保存→batch/梯度累加→碎片化→隐式计算图引用  
- **质量退化**：数据污染→评测泄露→过拟合→对齐税→RAG 检索召回不足
