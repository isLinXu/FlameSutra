...
## 本卷增强补全（2026）— 多模态对齐目标 · 评测与失败模式 · 文档/视频落地

> 本节为《焚诀》深度研究版的**回填内容**，补齐“CLIP/VLM 为什么有效、为什么会错、怎么落地到产业”。  
> 完整增强总览见：[焚诀-深度研究版-全卷增强补全（2026）](../焚诀-深度研究版-全卷增强补全.md)

### 1) CLIP 对比学习（InfoNCE）最小公式与直觉

CLIP 的核心：拉近“真配对图文”，推远“假配对图文”，把两种模态映射到同一语义空间：

\\[
\\mathcal{L}=-\\log\\frac{\\exp(\\text{sim}(i,t)/\\tau)}{\\sum_{t'}\\exp(\\text{sim}(i,t')/\\tau)}
\\]

其中 \\(\\tau\\) 是温度系数，控制分布“尖锐程度”。直觉：温度越小，模型越“挑剔”，训练更难但对齐更强。

### 2) VLM 必须补齐的评测与失败模式（培训读者最容易缺这块）

- **多模态幻觉**：看不清就瞎编细节（小字、密集表格尤甚）  
- **OCR/版面失败**：文档场景往往需要 OCR + 版面理解链路，而不是纯 VLM 直出  
- **空间关系/计数**：相对位置、数量、几何推理仍是常见短板  
- **安全与隐私**：图片中可能含 PII；以及“图片提示注入”（把攻击指令写进图里）

### 3) 三条产业落地路线（与第六卷合流）

1. **文档理解**：OCR/版面→结构化抽取→RAG（引用与权限）  
2. **电商与内容理解**：图像→属性抽取→生成描述/审核  
3. **视频理解入门**：抽帧→VLM→时间一致性校验（避免逐帧幻觉）
